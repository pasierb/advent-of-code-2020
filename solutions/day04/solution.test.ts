import { hasValidPassportFields, hasValidPassportData } from './solution';
...
describe('day04', () => {
  ...
  ])('part1', (passport: string, valid: boolean) => {
    it(`${passport} should be ${valid ? 'valid' : 'invalid'}`, () => {
      expect(hasValidPassportFields(passport)).toEqual(valid);
    });
  });
  ...
  ])('part2', (passport: string, valid: boolean) => {
    it(`${passport} should be ${valid ? 'valid' : 'invalid'}`, () => {
      expect(hasValidPassportFields(passport) && hasValidPassportData(passport)).toEqual(valid);
    })
  });
});
